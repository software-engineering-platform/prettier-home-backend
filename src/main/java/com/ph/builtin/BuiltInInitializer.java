...
public class BuiltInInitializer implements CommandLineRunner {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(BuiltInInitializer.class);
  ...
  private void initializeDefaultAdvert() {
    if (!advertRepository.existsBy()) {
      ...
      for (Map<String, Object> advert : adverts) {
        ...
        Image image = buildImage(advert.get("image").toString(), title, "static/rEuWY.jpg");
  ...
  /**
   * Builds an Image object using the provided fileName, title, and defaultImagePath.
   * If fileName is null or empty, defaultImagePath is used.
   *
   * @param fileName         The file name to read image data from
   * @param title            The title of the image
   * @param defaultImagePath The default image path to use if fileName is null or empty
  ...
  private Image buildImage(String fileName, String title, String defaultImagePath) {
    try {
      ...
      if (fileName != null && !fileName.isEmpty()) {
        ...
        data = downloadImageFromURL(defaultImagePath);
      }
      ...
      e.printStackTrace();
      LOGGER.warn("UNABLE TO READ JPG FILES");
    }
    return null;
  }
  ...
  /**
   * Downloads an image from a given URL and returns its content as a byte array.
   *
  ...
  private byte[] downloadImageFromURL(String urlString) throws IOException {
    ...
    // Open a connection to the URL and read all the bytes representing the image
